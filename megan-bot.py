...
async def setup_applications(interaction: discord.Interaction, channel: discord.TextChannel):
    ...
    # 🆕 V1.2: أرسل الصورة كرسالة منفصلة في الأعلى (مطابقة للتصميم المرجعي)
    # ثم أرسل الـ embed بعدها — هذا يجعل الصورة تظهر فوق النص مباشرة
    banner_sent_as_message = False
    if banner_url and os.path.exists(BANNER_IMAGE_LOCAL_PATH):
        try:
            with open(BANNER_IMAGE_LOCAL_PATH, "rb") as f:
                banner_file = discord.File(f, filename="megan_banner.png")
                await channel.send(file=banner_file)
                banner_sent_as_message = True
                logger.info(f"🖼️  Banner sent as separate message (top of channel)")
        except Exception as e:
            logger.warning(f"Failed to send banner as message: {e}")

    # 🆕 embed بدون صورة (الصورة فوقها كرسالة منفصلة)
    embed = discord.Embed(
    ...
    # 🆕 استخدم thumbnail لشعار السيرفر (إن وجد) — يظهر في الأعلى بجانب العنوان
    if interaction.guild.icon:
