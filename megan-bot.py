...
THUMBNAIL_IMAGE_LOCAL_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "megan-assets", "thumbnail.png")
...
async def setup_applications(interaction: discord.Interaction, channel: discord.TextChannel):
    ...
    # 🆕 V1.4: ارفع الصورة الكبيرة + الـ thumbnail كـ attachments
    # الصورة الكبيرة → set_image في embed منفصل (تظهر في الأعلى)
    # الـ thumbnail → set_thumbnail في embed النص (تظهر في الزاوية اليمنى)
    banner_file_obj = None
    thumbnail_file_obj = None
    if os.path.exists(BANNER_IMAGE_LOCAL_PATH):
        try:
            banner_file_obj = discord.File(BANNER_IMAGE_LOCAL_PATH, filename="megan_banner.png")
        except Exception as e:
            logger.warning(f"Failed to load banner file: {e}")
    if os.path.exists(THUMBNAIL_IMAGE_LOCAL_PATH):
        try:
            thumbnail_file_obj = discord.File(THUMBNAIL_IMAGE_LOCAL_PATH, filename="megan_thumbnail.png")
        except Exception as e:
            logger.warning(f"Failed to load thumbnail file: {e}")

    # 🆕 Embed 1: الصورة الكبيرة فقط (في الأعلى)
    if banner_file_obj:
        image_embed = discord.Embed(color=COLORS["primary"])
        image_embed.set_image(url="attachment://megan_banner.png")
        await channel.send(embed=image_embed, file=banner_file_obj)
        logger.info(f"🖼️  Banner embed sent (top)")

    # 🆕 Embed 2: النص + thumbnail (الصورة الصغيرة في الزاوية اليمنى)
    embed = discord.Embed(
    ...
    # 🆕 V1.4: استخدم الصورة الجديدة كـ thumbnail (بدل شعار السيرفر)
    if thumbnail_file_obj:
        embed.set_thumbnail(url="attachment://megan_thumbnail.png")

    view = ApplicationsView()
    # 🆕 V1.4: أرسل embed 2 مع ملف الـ thumbnail كـ attachment
    if thumbnail_file_obj:
        msg = await channel.send(embed=embed, view=view, file=thumbnail_file_obj)
    else:
...
if __name__ == "__main__":
    TOKEN = os.getenv("MEGAN_TOKEN") or "MTUyMTE5NDAwOTM1ODM2ODk2MA.GoayM5.H3XoL63J5KCvinogvN1Gbtm3GR0T9xG3SK7Cig"
    if not TOKEN:
        raise RuntimeError(
