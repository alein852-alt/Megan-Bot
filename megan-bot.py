...
async def setup_applications(interaction: discord.Interaction, channel: discord.TextChannel):
    ...
    # 🆕 V1.5: embed واحد فقط — الصورة الكبيرة + النص + الأزرار
    # الصورة تأخذ مساحة كبيرة (set_image)، والأزرار تكون أسفل الـ embed مباشرة
    ...
    # 🆕 Embed واحد فقط: الصورة + النص + (الأزرار تُضاف عند الإرسال)
    embed = discord.Embed(
    ...
    # 🆕 V1.5: الصورة الكبيرة في نفس الـ embed (set_image تظهر في الأسفل، فوق الأزرار)
    if banner_file_obj:
        embed.set_image(url="attachment://megan_banner.png")

    view = ApplicationsView()
    # 🆕 V1.5: أرسل embed واحد فقط مع الصورة الكبيرة كـ attachment
    if banner_file_obj:
        msg = await channel.send(embed=embed, view=view, file=banner_file_obj)
    else:
