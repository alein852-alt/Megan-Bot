...
# 🆕 V2.0: صورة الـ banner برابط مباشر — لا حاجة لملف محلي
BANNER_IMAGE_URL = "https://tmpfiles.org/dl/wzwGtnuawT4P/banner_compressed.jpg"
...
class ApplicationModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # 🆕 V2.0: أضف الصورة الزرقاء برابط مباشر في رسالة التأكيد
        success_embed.set_image(url=BANNER_IMAGE_URL)
...
async def setup_applications(interaction: discord.Interaction, channel: discord.TextChannel):
    ...
    # 🆕 V2.0 FINAL: صورتان متتاليتان + النص + الأزرار
    # تستخدم رابط مباشر للصورة — لا حاجة لملف محلي
    # الرسالة الأولى: صورة كبيرة (1)
    # الرسالة الثانية: صورة كبيرة (2) + النص + الأزرار

    # 🆕 رسالة 1: الصورة الأولى فقط (برابط مباشر)
    image_embed_1 = discord.Embed(color=COLORS["primary"])
    image_embed_1.set_image(url=BANNER_IMAGE_URL)
    await channel.send(embed=image_embed_1)
    logger.info(f"🖼️  Banner image 1 sent (URL)")

    # 🆕 رسالة 2: الصورة الثانية + النص + الأزرار
    # ✅ ULTIMATE: عمودين جنباً إلى جنب (إنجليزي + عربي) + صورة في الأعلى
    embed = discord.Embed(
        title="📋  Staff Applications",
        description=(
            f"> Read carefully  •  اقرأ بعناية\n"
            f"{separator()}"
        ),
        color=COLORS["primary"],
        timestamp=discord.utils.utcnow()
    ...
    # ✅ ULTIMATE: الصورة في الأعلى (thumbnail يظهر في الزاوية اليمنى العليا)
    embed.set_thumbnail(url=BANNER_IMAGE_URL)

    # 🆕 عمود 1: English (inline=True)
    embed.add_field(
        name="🇬🇧 Staff Requirements",
        value=(
            f"```\n"
            f"• Must be active & mature\n"
            f"• Good behavior & respect\n"
            f"• No abuse of power\n"
            f"• Enforce server rules\n"
            f"• No staff drama\n"
            f"• Listen to higher staff\n"
            f"• Experience is a plus\n"
            f"```"
        ),
        inline=True
    )

    # 🆕 عمود 2: العربية (inline=True)
    embed.add_field(
        name="🇸🇦 شروط الانضمام",
        value=(
            f"```\n"
            ...
            f"• تطبيق قوانين السيرفر\n"
            f"• لا دراما ولا محاباة\n"
            f"• الاستماع للفريق الأعلى\n"
            f"• خبرة سابقة ميزة\n"
            f"```"
        ),
        inline=True
    ...
    # 🆕 صف كامل: How to Apply (inline=False)
    embed.add_field(
        name="📝  How to Apply  •  كيفية التقديم",
        value=(
            f"> 🇬🇧  Press a button below and answer the questions\n"
            f"> 🇸🇦  اضغط على أحد الأزرار بالأسفل وأجب على الأسئلة\n"
            f"{separator()}\n"
            f"> ⚠️  Any abuse or lying = ban  •  أي كذب قد يؤدي إلى حظر  💖"
        ),
        inline=False
    )

    embed.set_footer(text=f"{BOT_FOOTER}  •  Staff Applications  •  ULTIMATE")
    # 🆕 الصورة الثانية في الأسفل (set_image)
